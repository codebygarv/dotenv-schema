...
${pc2.bold(data.score >= 80 ? pc2.green(data.score) : data.score >= 60 ? pc2.yellow(data.score) : pc2.red(data.score))}/100
`);
    ...
    if (data.unusedVariables.length > 0) {
      console.log(pc2.yellow(`\u26A0 ${data.unusedVariables.length} variables appear unused:`));
      data.unusedVariables.forEach((v) => console.log(pc2.dim(`  - ${v}`)));
    } else {
...
\u2717 ${data.missingVariables.length} variables used in code but missing from environment:`));
      data.missingVariables.forEach((v) => console.log(pc2.dim(`  - ${v}`)));
    }
...
\u2713 No security issues detected.`));
    ...
    if (data.missingVariables.length > 0 || data.securityCriticals > 0) {
      console.error(pc2.red(`\u2717 Audit failed: Found ${data.missingVariables.length} missing variables and ${data.securityCriticals} critical security issues.`));
      process.exit(1);
    }
  } catch (err) {
